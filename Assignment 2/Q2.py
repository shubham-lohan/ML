...
def pre_process():
    ...
    df = df.to_numpy()
    n = df.shape[0]
    X_train = df[:int(n*0.7), 1:]/255
    y_train = df[:int(n*0.7), 0]

    # print(df)
    X_test = df[int(n*0.7):-int(n*0.2), 1:]/255
    y_test = df[int(n*0.7):-int(n*0.2), 0]

    X_validate = df[-int(n*0.2):, 1:]/255
    y_validate = df[-int(n*0.2):, 0]
